...
static void send_packet(int usd, u32 time_sent[2], struct ntp_control*);
...
static void set_time(struct ntptime *new, struct ntp_control* ntpc)
{
	...
	tv_set.tv_sec  = new->coarse - JAN_1970 + ntpc->timezone + ntpc->dst_offset;
	/* divide xmttime.fine by 4294.967296 */
	tv_set.tv_nsec = USEC(new->fine)*1000;
	if (clock_settime(CLOCK_REALTIME, &tv_set)<0) {
		ntpc->clock_was_set = 0;
	} else {
		ntpc->clock_was_set = 1;
		if (debug) {
	...
	struct timeval tv;
	/* it would be even better to subtract half the slop */
	tv.tv_sec  = new->coarse - JAN_1970 + ntpc->timezone + ntpc->dst_offset;
	/* divide xmttime.fine by 4294.967296 */
	tv.tv_usec = USEC(new->fine);
	rtc_tv2rd(&tv, &rd);

	if (rtc_set_clock(&rd)<0) {
		ntpc->clock_was_set = 0;
	} else {
		ntpc->clock_was_set = 1;
		if (debug) {
...
static void ntpc_gettime(struct ntp_control* ntpc, u32 *time_coarse, u32 *time_fine)
{
#ifndef USE_OBSOLETE_GETTIMEOFDAY
	...
	*time_fine   = NTPFRAC(now.tv_nsec/1000);
	if (ntpc->clock_was_set) {
		*time_coarse -= ntpc->timezone - ntpc->dst_offset;
	}
#else
	...
	*time_fine   = NTPFRAC(now.tv_usec);
	if (ntpc->clock_was_set) {
		*time_coarse -= ntpc->timezone - nptc->st_offset;
	}
#endif
}

static void send_packet(int usd, u32 time_sent[2], struct ntp_control* ntpc)
{
	u32 data[12];
#define LI 0
#define VN 3
...
#define PREC -6
	...
	ntpc_gettime(ntpc, time_sent, time_sent+1);
	data[10] = htonl(time_sent[0]); /* Transmit Timestamp coarse */
	data[11] = htonl(time_sent[1]); /* Transmit Timestamp fine   */
	send(usd,data,48,0);
}

static void get_packet_timestamp(int usd, struct ntptime *udp_arrival_ntp, struct ntp_control* ntpc)
{
#ifdef PRECISION_SIOCGSTAMP
	struct timeval udp_arrival;
	if ( ioctl(usd, SIOCGSTAMP, &udp_arrival) < 0 ) {
		perror("ioctl-SIOCGSTAMP");
		ntpc_gettime(ntpc, &udp_arrival_ntp->coarse, &udp_arrival_ntp->fine);
	} else {
...
static void setup_transmit(int usd, short port, struct ntp_control *ntpc)
{
	struct sockaddr_in sa_dest;
	memset(&sa_dest, 0, sizeof sa_dest);
	sa_dest.sin_family=AF_INET;
	stuff_net_addr(&(sa_dest.sin_addr),ntpc->hostname);
	...
	for (;;) {
		...
		if ((i!=1)||(!FD_ISSET(usd,&fds))) {
			...
			if (to.tv_sec == 0) {
				...
				send_packet(usd,ntpc->time_of_send, ntpc);
				++probes_sent;
		...
		if (pack_len<0) {
			...
			get_packet_timestamp(usd, &udp_arrival_ntp, ntpc);
...
void read_config(struct ntp_control* ntpc) {
	FILE* cfg = fopen("/etc/ntpclient/ntpc.conf", "r");
	if (!cfg) {
		return;
	}
...
int main(int argc, char *argv[]) {
	...
	memset(&ntpc, 0, sizeof(struct ntp_control));
	...
	for (;;) {
		c = getopt( argc, argv, "c:z:o:" DEBUG_OPTION "f:g:h:i:lp:q:" REPLAY_OPTION "st");
		if (c == EOF) break;
		switch (c) {
			case 'c':
			...
			case 'g':
				...
				break;
			case 'h':
				snprintf(ntpc.hostname, sizeof(ntpc.hostname), "%s", optarg);
				break;
			case 'i':
	...
	if (debug) {
		...
		ntpc.hostname, ntpc.cycle_time, ntpc.live, udp_local_port, min_delay,
		ntpc.set_clock, ntpc.cross_check, ntpc.timezone, ntpc.dst_offset);
	}
	...
	setup_transmit(usd, NTP_PORT, &ntpc);
